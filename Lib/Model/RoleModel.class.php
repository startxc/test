...
class RoleModel extends CommonModel {
    ...
    function setGroupApps($groupId, $appIdList) {
        ...
        $result = $this->db->execute('INSERT INTO ' . $this->tablePrefix . 'rbac_access (role_id,node_id,pid,level) SELECT a.id, b.id,b.pid,b.level FROM ' . $this->tablePrefix . 'rbac_role a, ' . $this->tablePrefix . 'rbac_node b WHERE ' . $where);
        if ($result === false) {
            return false;
        } else {
            return true;
    ...
    function getGroupAppList($groupId) {
        $rs = $this->db->query('select b.id,b.title,b.name from ' . $this->tablePrefix . 'rbac_access as a ,' . $this->tablePrefix . 'rbac_node as b where a.node_id=b.id and  b.pid=0 and a.role_id=' . $groupId . ' ');
        return $rs;
    }

    function delGroupApp($groupId) {
        $table = $this->tablePrefix . 'rbac_access';
    ...
    function getGroupModuleList($groupId, $appId) {
        $table = $this->tablePrefix . 'rbac_access';
        $rs = $this->db->query('select b.id,b.title,b.name from ' . $table . ' as a ,' . $this->tablePrefix . 'rbac_node as b where a.node_id=b.id and  b.pid=' . $appId . ' and a.role_id=' . $groupId . ' ');
        return $rs;
    }
    ...
    function setGroupModules($groupId, $moduleIdList) {
        ...
        $rs = $this->db->execute('INSERT INTO ' . $this->tablePrefix . 'rbac_access (role_id,node_id,pid,level) SELECT a.id, b.id,b.pid,b.level FROM ' . $this->tablePrefix . 'rbac_role a, ' . $this->tablePrefix . 'rbac_node b WHERE ' . $where);
        if ($result === false) {
            return false;
        } else {
            return true;
    ...
    function getGroupActionList($groupId, $moduleId) {
        $table = $this->tablePrefix . 'rbac_access';
        $rs = $this->db->query('select b.id,b.title,b.name from ' . $table . ' as a ,' . $this->tablePrefix . 'rbac_node as b where a.node_id=b.id and  b.pid=' . $moduleId . ' and  a.role_id=' . $groupId . ' ');
        return $rs;
    }
    ...
    function getGroupUserList($groupId) {
        $table = $this->tablePrefix . 'rbac_role_user';
    ...
    function setGroupUser($groupId, $userId) {
        $sql = "INSERT INTO " . $this->tablePrefix . 'rbac_role_user (role_id,user_id) values (' . $groupId . ',' . $userId . ')';
    ...
    function setGroupUsers($groupId, $userIdList) {
        ...
        $rs = $this->execute('INSERT INTO ' . $this->tablePrefix . 'rbac_role_user (role_id,user_id) SELECT a.id, b.id FROM ' . $this->tablePrefix . 'rbac_role a, ' . $this->tablePrefix . 'user b WHERE ' . $where);
        if ($result === false) {
            return false;
        } else {
            return true;
