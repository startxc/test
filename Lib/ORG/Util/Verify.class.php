...
class Verify {
	...
	public function check($code, $id = '') {
		$key = Verify::authcode($this->seKey);
		// 验证码不能为空
		$session = session($key);
		...
		if(Verify::authcode(strtoupper($code)) == $secode['code']) {
			session($key, null);
			return true;
		}
	...
	public function entry($id = '') {
		...
		// 验证码使用随机字体
		$ttfPath = ROOT_PATH."Public/Font/". ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
		...
		// 保存验证码
		$key = Verify::authcode($this->seKey);
		$code = Verify::authcode(strtoupper(implode('', $code)));
		$session = array();
		if($id) {
