...
// +----------------------------------------------------------------------
// | Author: liu2-1st <liu21st@gmail.com>
// +----------------------------------------------------------------------
...
class Images {
    ...
    function showImg($imgFile, $text = '', $x = '10', $y = '10', $alpha = '50') {
        ...
        $info = Images::getImageInfo($imgFile);
        if ($info !== false) {
            ...
            if ($im) {
                ...
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = Images::getImageInfo($text);
    ...
    static function buildString($string, $rgb = array(), $filename = '', $type = 'png', $disturb = 1, $border = true) {
        ...
        Images::output($im, $type, $filename);
    }
  ...
  static function buildImageVerify($length=5, $mode=1, $type='png', $imageL=0, $imageH=0, $verifyName='verify',$fontSize=25,$fontttf='') {  // 验证码字体，不设置随机获取  抄袭的 onethink
        ...
        // 验证码使用随机字体
        $ttfPath = ROOT_PATH."Public/Font/ttfs/";
    ...
    //图片裁剪
    static function dothumb($srcfile, $dstfile, $thumbwidth, $thumbheight, $maxthumbwidth = 0, $maxthumbheight = 0, $src_x = 0, $src_y = 0, $src_w = 0, $src_h = 0) {
        if (!is_file($srcfile)) {
            return '';
        }
        $tow = $thumbwidth;
